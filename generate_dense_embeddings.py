    ...
    for j, batch_start in enumerate(range(0, n, bsz)):
        ...
        if cfg.viewer_number > 1:
            ctx_position_ids = torch.cat((torch.zeros(cfg.viewer_number-1),torch.arange(ctx_ids_batch.size(1)-cfg.viewer_number+1)),0) \
                .expand(ctx_ids_batch.size(0),-1).to(dtype=torch.long)
            ctx_position_ids = move_to_device(ctx_position_ids, cfg.device)
        else:
            ctx_position_ids = None

        with torch.no_grad():
            # _, out, _ = model(ctx_ids_batch, ctx_seg_batch, ctx_attn_mask)
            out, _, _ = model(ctx_ids_batch, ctx_seg_batch, ctx_attn_mask, ctx_position_ids)

        viewer_number=cfg.viewer_number
        if viewer_number > 1:
            out = out[:, :viewer_number, :].cpu()
        else:
            out = out[:, 0, :].cpu()
        ...
        if viewer_number > 1 and cfg.model_mode =="mvr":
            if extra_info:
                results.extend(
                    [
                ...
                results.extend(
                    [(ctx_ids[i], out[i,j,:].view(-1).numpy()) for i in range(out.size(0)) for j in range(out.size(1))]
                )
        else:
