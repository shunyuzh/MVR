...
class BiEncoderTrainer(object):
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            biencoder_input = BiEncoder.create_biencoder_input2(
                ...
                num_other_negatives,
                shuffle=False,
                viewer_number=cfg.viewer_number,
            )
    ...
    def validate_average_rank(self) -> float:
        ...
        if cfg.loss_mode == "dot":
            sim_score_f = BiEncoderNllLoss.get_similarity_function("dot")
        else:
            sim_score_f = BiEncoderNllLoss.get_similarity_function("max_dot")
        q_represenations = []
        ctx_represenations = []
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):
                ...
                q_ids, q_segments, q_position_ids = (
                    (biencoder_input.question_ids, biencoder_input.question_segments, biencoder_input.question_position_ids)
                    if j == 0
                    else (None, None, None)
                )
                ...
                if cfg.viewer_number > 1:
                    ctx_position_ids = torch.cat((torch.zeros(cfg.viewer_number-1),torch.arange(ctx_ids_batch.size(1)-cfg.viewer_number+1)),0) \
                        .expand(ctx_ids_batch.size(0),-1).to(device=ctx_attn_mask.device, dtype=torch.long)
                else:
...
def _do_biencoder_fwd_pass(
    ...
    if model.training:
        model_out = model(
            ...
            q_attn_mask,
            input.question_position_ids,
            input.context_ids,
            input.ctx_segments,
            ctx_attn_mask,
            input.context_position_ids,
            encoder_type=encoder_type,
            representation_token_pos=rep_positions,
            viewer_number=cfg.viewer_number,
            model_mode=cfg.model_mode,
