    ...
    with torch.no_grad():
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            if selector:
                ...
                out, _, _ = BiEncoder.get_representation(
                    question_encoder,
                    q_ids_batch,
                    q_seg_batch,
                    q_attn_mask,
                ...
                # selector is None unless set in cfg
                out, _, _ = question_encoder(q_ids_batch, q_seg_batch, q_attn_mask)
                out=out[:, 0]
...
def main(cfg: DictConfig):
    ...
    for i, ds_item in enumerate(qa_src.data):
    ...
    index.init_index(vector_size, True)
    ...
    n_docs = cfg.n_docs * cfg.viewer_number
    ...
    buffer_number=0
    for i, item in enumerate(
            iterate_encoded_files(input_paths, path_id_prefixes=path_id_prefixes)
    ):
        buffer.append(item)
        if 0 < buffer_size == len(buffer):
            index.index_data(buffer)
            buffer_number +=1
            if buffer_number % check_number==0:
                logger.info("Data indexing completed.")
                top_ids_and_scores = retriever.get_top_docs(questions_tensor.numpy(), n_docs)
                if not result:
                    result = [list(a) for a in top_ids_and_scores]
                else:
                    for j, doc_tuple in enumerate(top_ids_and_scores):
                        result[j][0].extend(doc_tuple[0])
                        result[j][1]=list(result[j][1])
                        result[j][1].extend(list(doc_tuple[1]))
                        dict_rank=zip(result[j][0], result[j][1])
                        dict_rank = sorted(dict_rank, key = lambda i: i[1], reverse=True)[:n_docs]
                        result[j] = [list(a) for a in zip(*dict_rank)]
                del index
                retriever=None
                index = hydra.utils.instantiate(cfg.indexers[cfg.indexer])
                index.init_index(vector_size, True)
    ...
    top_ids_and_scores = [tuple(a) for a in result]
    new_closest_docs=[]
    for i, doc_tuple in enumerate(top_ids_and_scores):
        id_list=[]
        score_list=[]
        for doc_id in doc_tuple[0]:
            if doc_id not in id_list:
                id_list.append(doc_id)
                loc = id_list.index(doc_id)
                score_list.append(doc_tuple[1][loc])
        new_closest_docs.append((id_list[:100], score_list[:100]))
    top_ids_and_scores = new_closest_docs
