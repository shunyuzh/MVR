...
def get_bert_tensorizer(cfg, tokenizer=None):
    ...
    return BertTensorizer(tokenizer, sequence_length, viewer_number, model_mode)
...
class BertTensorizer(Tensorizer):
    def __init__(
        self, tokenizer: BertTokenizer, max_length: int, viewer_number: int, model_mode: str, pad_to_max: bool = True
    ):
    ...
    def text_to_tensor(
        ...
        # Add multiple viewers. Original token_ids = [cls, word, word, ... , word]
        # pad=0, cls=101, seq=102, unused1=1, unused2=2
        if self.model_mode=='mvr':
            if enable_query:
                add_sents = []
                token_ids[0] = 1
            else:
                add_sents = [ i+1 for i in range(self.viewer_number-1)]
                token_ids[0] = self.viewer_number
            add_sents.extend(token_ids)
            token_ids = add_sents
