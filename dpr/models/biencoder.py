...
def global_local_loss(q_vectors: T, ctx_vectors: T, lamda, epoch, temperature):
    # q_vector: n1 x D, ctx_vectors: n2 x viewer_number x D,  finally result [n1, n2]
    scores = torch.matmul(q_vectors, torch.transpose(ctx_vectors, 1, 2)).transpose(0,1)   # result [n1, n2, obs_number]
    t = max(0.3, math.exp(-0.1 * epoch))
    scores /= t
    r, _ = torch.max(scores, dim=2)
    r = F.log_softmax(r, dim=1)
    # plus max diversity in [viewer]
    plus = F.log_softmax(scores, dim=2)
    plus, _ = torch.max(plus, dim=2)
    r = r + lamda * plus
    return r
...
class BiEncoder(nn.Module):
    ...
    def forward(
        ...
        _ctx_seq, ctx_pooled_out, _ctx_hidden = self.get_representation(
            ctx_encoder, context_ids, ctx_segments, ctx_attn_mask,
            ctx_position_ids, self.fix_ctx_encoder
        )

        if model_mode=='cls':
            ctx_pooled_out = _ctx_seq[:, 0, :]
            if _q_seq is not None:
                q_pooled_out = _q_seq[:, 0, :]
        elif model_mode=='mvr':
            ctx_pooled_out = _ctx_seq[:, :viewer_number, :]
            if _q_seq is not None:
                q_pooled_out = _q_seq[:, 0, :]
    ...
    def create_biencoder_input2(
        ...
        length = ctxs_tensor.size(1)
        bs = ctxs_tensor.size(0)
        question_postion_ids = None
        if viewer_number > 1:
            ctx_position_ids = torch.cat((torch.zeros(viewer_number-1),torch.arange(length-viewer_number+1)),0)\
                .expand(bs,-1).to(device=ctx_segments.device, dtype=torch.long)
        else:
...
class BiEncoderNllLoss(object):
    def calc(
        ...
        """
        softmax_scores = self.get_scores(q_vectors, ctx_vectors, loss_mode, lamda, epoch, temperature)

        if len(q_vectors.size()) > 1:
            q_num = q_vectors.size(0)
            softmax_scores = softmax_scores.view(q_num, -1)
    ...
    @staticmethod
    def get_scores(q_vector: T, ctx_vectors: T, loss_mode: str, lamda, epoch, temperature) -> T:
        f = BiEncoderNllLoss.get_similarity_function(loss_mode)
        if "global_local" in loss_mode:
            return f(q_vector, ctx_vectors, lamda, epoch, temperature)
        else:
            return f(q_vector, ctx_vectors)

    @staticmethod
    def get_similarity_function(loss_mode):
        if loss_mode=='dot':
            return dot_product_scores
        elif loss_mode=="global_local_loss":
            return global_local_loss
