...
BiEncoderPassagePAQ = collections.namedtuple("BiEncoderPassagePAQ", ["text", "title", "query"])
class CsvCtxSrcPAQ(RetrieverData):
    def __init__(
            self,
            file: str,
            id_col: int = 0,
    ...
    def load_data_to(self, ctxs: Dict[object, BiEncoderPassagePAQ]):
        super().load_data()
        with open(self.file) as ifile:
            reader = csv.reader(ifile, delimiter="\t")
            for row in reader:
                ...
                if self.normalize:
                    passage = normalize_passage(passage)
                ctxs[sample_id] = BiEncoderPassagePAQ(passage, row[self.title_col], row[self.query_col])
